...
public class NeutronHttpApiModule extends HttpApiModule<NeutronApi> {
    ...
    @Provides
    @Singleton
    public LoadingCache<String, Set<? extends Extension>> provideExtensionsByZone(final Provider<NeutronApi> NeutronApi) {
        return CacheBuilder.newBuilder().expireAfterWrite(23, TimeUnit.HOURS)
                .build(new CacheLoader<String, Set<? extends Extension>>() {
                    @Override
                    public Set<? extends Extension> load(String key) throws Exception {
                        return NeutronApi.get().getExtensionApiForZone(key).list();
                    }
                });
    }
